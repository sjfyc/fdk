...
namespace fdk { namespace game { namespace navi
{
	...
	bool GridBasedEnv::isDirectlyReachable(int startNodeID, int targetNodeID) const
	{
		...
		const NodeCoord startNodeCoord = toNodeCoord(startNodeID);
		const NodeCoord targetNodeCoord = toNodeCoord(targetNodeID);
		...
		for (int i = 1; i <= dx; ++i)
		{
			...
			if (!isValidNodeCoord(nodeCoord) || !isNodeWithCoordReachable(nodeCoord) )
			{
				return false;
			}
	...
	int GridBasedEnv::getFirstDirectlyReachableNode(int startNodeID, int targetNodeID) const
	{
		if (startNodeID == targetNodeID)
		{
			return startNodeID;
