...
private:
  struct luaproxy { /* FullUserData: 只有成员: 实体类指针 */
    person *impl;
  };
...
void person::_regtolua(lua_State *L)
{
  ...
  pl = (luaproxy *)lua_newuserdata(L, sizeof(luaproxy));
  pl->impl = this;          /* stack: fud */
  ...
  luaL_register(L, NULL, lib);
                            ...
                            /* stack: fud mt */
  lua_setmetatable(L, -2);  /* stack: fud */
...
{  
  ...
  pl = luaL_checkudata(L, idx, PERSON_NAME_IN_LUA);
  luaL_argcheck(L, pl != NULL, idx, PERSON_ARG_CHECK_FAILED_MSG);

  return ((luaproxy *)pl)->impl;
}
